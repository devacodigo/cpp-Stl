...
auto it =mpp.find("simran");//points to end since she doesnt exist
if (mpp.empty()) {
...
pair<int, int> pr;//stores twp thingss together like raj assigned no.29
pr.first = 1;
pr.second = 10;
...
}//prints the whole map (all key and values)

 for (auto it = mpp.begin(); it!=mpp.end ();it++) {
    cout <<it->first <k " " « it->second << endl;

}


unordered_map<int,int>map;//does not stores in any order
//unorderd map cannot store pair as a ker like unorderd_map<pair<int,int>,int>mpp;
//you need to use map for storing pair as a key
...
// Stack and Queue 
    ...
    //there does not exist a clear function in stack
	...
	stack<int> st; //this if statement is very impotant to avoid run time error
	if(!st.empty()) {
    ...
    //there is no itreator in stack quaue
 
	// priority_queue
     //all in sorted order//opertions in logn
	// push 
	// size 
	// top pop empty 
    //duplicates are allowsed
	priority_queue<int> pq;
	pq.push(1); 
	pq.push(5); 
	pq.push(2); 
	pq.push(6); 
 //pq uses heapify concept
	cout << pq.top(); // print 6 
	pq.pop(); 
	cout << pq.top(); // print 5 
    ...
    //if you want a least priority queue
	priority_queue<int> pq;
	...
	priority_queue<pair<int,int>, vector<pair<int,int>>, greater<pair<int,int>>> pq; // its a syntax
 ...
 // Bitset 
	...
	// bitset -> 1 bit //8tims smallerr thsn smalllest possibl datatype
	...
	// flip 
	// bt -> 10100
	bt.flip(2); // bt will become 10000 //flipped the bit at 2 index
	...
	// set 
	bt.set(); // 11111 //irrespective of what it had
 
	bt.set(2); // sets the 2nd index with 1
 
	bt.set(2, 0); //sets 0 at 2nd index
	...
	// Algorithms 
	// 1.sorting 
	// array, vector (linear sequancial fashion)
	int n;
	cin >> n; 
	int arr[n]; 
    ...
    reverse(vec.begin() + 1, vec.begin() + 4); //reverses elements from index 1 to 3 only
    ...
    //below is the internal implimentations
    int maxi = INT_MIN; 
    for(int k = i;k<=j;k++) {
    	if(a[k] > maxi) {
    		maxi = a[k]; 
    ...
    int cnt = count(arr, arr+n, 1);//gives the count of value 1
    ...
    // arr[] -> {1, 5, 6, 2, 3, 5, 6}
    // x = 4 
    auto it = find(vec.begin(), vec.end(), 4); //pointing to end or address of elemnet after last element
    if(it == vec.end()) {
    ...
    // x = 10 ->points to first 10
    // x = 6 ->points to fist element which is not lesser than 6 which is 7 in the array
    // x = 13 ->points to last iterator 
    // this works in log N 
